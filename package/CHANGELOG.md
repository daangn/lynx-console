...
- 7f55d7e: 위로 스크롤하거나 드래그하면 자동 따라가기를 멈추고, 맨 아래 이동 버튼으로 다시 시작할 수 있어요. 연속 로그 중에도 잠금을 유지하고, 잠금 시점의 전체 항목 ID를 기준으로 새 로그를 집계해 필터 변경으로 나타난 기존 항목은 제외해요.
- 6365df6: 탭을 다중 선택 필터로 바꿨어요. 여러 탭을 동시에 켜서 합집합으로 보고, 아무것도 안 켜면 전부 보여줘요.
...
- 455c569: Require devalue 5.9.2 or later to include fixes for deserialization denial-of-service vulnerabilities.
